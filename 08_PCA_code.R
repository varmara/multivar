...
X_back_pc1 <- X %*% E$vectors[, 1] %*% t(E$vectors[, 1]) # только PC1
gg_back_pc1 <- gg_centered %+% as.data.frame(X_back_pc1) +
  aes(x = V1, y = V2) +
  labs(x = "width", y = "length")

plot_grid(gg_centered, gg_back_full + ggtitle("Восстановленные"),
          gg_back_pc1 + ggtitle("Редуцированные"),nrow = 1, align = "h")
