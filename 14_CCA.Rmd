...
```{r mite-maps, echo = FALSE, message = FALSE, results='hide', fig.height = 7, fig.width = 14, cache=TRUE}
...
# Разновидности градиентного анализа 
...
В общем виде каноничесий анализ сводится к тому же, чо мы видели в CA и PCA, то есть к поиску собственных значений (задающих оси максимальноо варьирования) и собственных векторов (коэффициентов, определяющих координаты в новом пространстве).

НО! Оси ордиации определяются переменными среды.

##Принципиальная схема канонического анализа {.flexbox .vcenter} 
Этот анализ совмещает в себе регрессионный и корреспондентный анализа.
