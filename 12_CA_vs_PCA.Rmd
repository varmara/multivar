...
Обилие 102 видов птиц в 37 сайтах в юго-восточной Австралии (Mac Nally, 1989). Можно ли описать отношения между сайтами небольшим числом главных компонент?

```{r}
library(XLConnect)
birds <- readWorksheetFromFile(file="macnally.xlsx", sheet = 1)
# str(birds)
```
...
## Факторные нагрузки у многих переменных велики сразу на две оси
```{r}
...
```{r fig.width = 12, fig.height = 5, results='hold'}
op <- par(mfrow = c(1, 3), cex = 1)
biplot(bird_pca, display = "sites", scaling = 1, main = "PCA, без трансформации")
plot(procrustes(bird_h_pca, bird_pca), main = "Прокрустово преобразование")
biplot(bird_h_pca, display = "sites", scaling = 1, main = "PCA, трансформация Хеллингера")
par(op)
```
...
Вместо столбцов и строк исходных данных получают новые переменные - главные оси (principal axes)
...
```{r ratplots}
op <- par(mfrow = c(1, 2), mar = c(4, 4, 2, 1), cex = 1.2)
...
На графике CA видно, что в нескольких местах больше R.rattus и M.musculus, чем ожидается (это интродуценты). В нескольких других сайтах сильно различаются наблюдаемые и ожидаемые численности "родных" видов. Сайты с нефрагментированной территории располагаются вперемешку с сайтами с фрагментированной территории. Эффект "дуги" (так называется "эффект подковы" для CA) не заметен, т.к. экологический градиент, видимо, оказался довольно коротким.
...
```{r results='hold', tidy=TRUE, fig.height=5}
birds_ca <- cca(birds[ , -c(1, 2)])
op <- par(mfrow = c(1, 2))
col_ord_plot(ord = birds_h_pca, colfac = factor(birds$habitat), leg.pos = "bottomleft", ncol = 1, display.labs = FALSE)
col_ord_plot(ord = birds_ca, colfac = factor(birds$habitat), leg.pos = "bottomleft", ncol = 1, display.labs = FALSE)
par(op)
```
