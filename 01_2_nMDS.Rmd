...
## Графическое представление результатов средствами `ggplot2` {.columns-2}
...
ggplot(ord_MDS, aes(x = MDS1, y = MDS2)) + 
  geom_point(aes(fill = hydrol$Depth), size = 4, shape = 21) + 
...
ggplot(data.frame(ord1$points), aes(x = MDS1, y = MDS2)) + 
  geom_point() + 
<<<<<<< HEAD
  geom_text(aes(label = 
                  row.names(ord1$points)), vjust = -0.5)
=======
  geom_text(aes(label = row.names(ord1$points)), vjust = -0.5)
>>>>>>> f09cb7858b2a658467e8455cd083dddf0268169c
vegdist(red_abund[])
```
...
Для исходной, не случайной, ординации стресс сотавлял Stress = `r ord1$stress`


## Алгоритм MDS {.columns-2}
...
## Диаграмма Шепада для реальных данных

Строим диаграмму Шепарда вместе с монотонной регрессией на материале Долгой губы. 
