...
## Вспомним основы {.columns-2 .smaller}
...
## Наблюдается ли многолетний тренд в динамике сообщества, связанного с мидиевой банкой, и динамикой размерой структуры вида-эдификатора? {.columns-2}
...
В датасете `com` содержатся усреденные данные по обилию `r ncol(com[,-c(1:3)])`
 видов для 3 мидиевых банок (Vor2, Vor4, Vor5).    

В датасете `ascam` (averaged size class abundance matrix) содержатся средние плотноси поселения мидий разных размеров (`r ncol(ascam[, -c(1:2)])`] размерных класов)   

**Вопрос** Существуют ли направленые изменения в разменой структуре поселений мидий и в структуре сообщества?   


<img src="images/musselbed.png" width="450" height="350" alt="Мидиевая банка">


##Задание {.smaller .columns-2}
...
Это матрица эвклидовых расстояний между временными точками. 
...
## Протестируем гипотезу о  наличии временного градиента c с помощью теста Мантела {.smaller .columns-2}

```{r}
dist_vor2_com <- vegdist(vor2_log_com, 
                         method = "bray")
dist_vor2_ascam <- vegdist(vor2_log_ascam, 
                           method = "euclidean")
```

Наличие градиента в структуре сообщества   
```{r}
mantel(dist_vor2_com, gradient_model)
```

Наличие градиента в размерной структуре мидий
```{r}
...
## Могут быть и более сложные модельные матрицы {.smaller .columns-2}
...
##Выявляется ли циклическая составляющая в динамике размерной струкутры?
...
Можно обсуждать только цклы с периодом не более половины длины временного ряда.
...
- Постройте ординацию всех описаний датасета `com` (логарифмированные данные) в осях nMDS на основе матрицы Брея-Кёртиса
...
## Ограничения (Assumptions) для пирменения ANOSIM {.smaller .columns-2}
...
При проверке гиптезы о значимости различий между группами можно использовать тест Мантела. 

В этой ситуации модельная матрица будет содержать 0, если расстояние внутригрупповое, и 1 если рсстояние межгрупповое. 
