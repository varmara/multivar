...
- Обилия видов (M видов x N описаний) и параметры среды (K параметров x N описаний) 
- Морфометрические показатели (M признаков x N объектов) и генетические признаки (Экспрессия K геов x N объектов)
- Признаки  хозяина (M признаков x N объектов) и признки паразита (K признаков x N объектов) 

--- &twocol

# Ординация растительности пастбищах северных оленей
...
<img src="figure/Vare et al 1995 map.png" width="400" height="400 align="middle">    
...
```{r, echo=FALSE,  message=FALSE, results='hide', fig.height=9 }
...
<img src="figure/Mantel.png" width="400" height="600 align="middle">


--- &twocol


*Если две матрицы сопряжены, то меры сходства/различия в одной матрице должны быть подоюбы мерам сходства/различия в другой матрице* 
...
<img src="figure/Mantel princip.png" width="800" height="600 align="middle">
...
---
# Для оценки достоверности мантеловской корреляции применяется пермутационная процедура. Эта процедура реализована в функции `mantel()` в пакете `{vegan}` 
...
>- 1. Вычисляется матрица взаимных расстояний между объектами для зависимой матрицы $D_{BIO}$. Используются все ее перменные. 
>- 2. Матрица-предиктор имеет $p$ переменных. Вычисляются все возможные матрицы взаимных расстояний между объектами для всех возможных комбинаций признаков матрицы ENV - $D_{ENV_i}$. ВНИМАНИЕ! Таких матриц будет $2^p-1$. 
>- 3. Между каждой из матриц $D_{ENV_i}$ и матрицей $D_{BIO}$ вычисляется мантеловская корреляция. 
>- 4. Находится матрица  $D_{ENV_i}$, имеющая максимальное значение мантеловской корреляции.
>- 5. Выводятся признаки матрицы ENV, на основе которых получена максимально подобная матрица $D_{ENV_i}$. 
...
<img src="figure/BIOENV.png" width="400" height="600 align="middle">
