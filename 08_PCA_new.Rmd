...
opts_chunk$set(fig.align='center', tidy = FALSE, fig.width = 7, fig.height = 2.8, warning = FALSE, message = FALSE)
```
...
Исходные переменные скоррелированы. Хочется найти такую трансформацию исходных данных, чтобы "новые" переменные:
...
>- Что это напоминает? Матрицу собственных векторов  
$cov(\mathbf{XP}) =  
\begin{pmatrix}
var_{1} & 0 & \cdots & 0 \\
0 & var_{2} & \cdots & 0 \\    
...
```{r echo=FALSE, fig.height=5, fig.width=8, purl=FALSE}
nMDS <- ggplot(as.data.frame(metaMDS(protein[, -c(1:2)], trace = 0)$points), aes(x=MDS1, y=MDS2)) + geom_point(aes(color = protein$region)) + coord_equal()
