...
## Способ 1. Геометрическое описание (линейная алгебра). {.columns-2}
...
Для таких случаев необходима нормализация величин.  
Этот прием наиболее вжен для геометрического способа представления многомерных данных.

$$X_{stand} = \frac{X_i - \bar{X}}{\sigma_x}$$

>- Вопрос: Какими свойcтвами обладают нормализованные величины? 
...
$$X_{rel} = \frac{X_i}{\sum X_i} 100%$$
$$X_{rel} = \frac{X_i}{max(X_i)} 100%$$
...
scatterplot3d(x = protein$Eggs, y = protein$Cereals, z = protein$Fish, scale.y = 5, pch = 21, xlab = "Eggs", ylab = "Cerials", zlab = "Fish")
...
## Задание: На основе датасета `protein` создайте матрицу, содержащую относительные величины (доли протеина каждого типа в общей количестве потребляемых протеинов)
...
Меры сходства/различия не учитывающие двойные нули. Эти коэффициенты не изменяются если в данные будут добавлены двойные нули (например, при увеличеннии количества описанных объектов). Примеры: Евклидово расстояние, расстояние по Манхеттену.  

Меры сходства/различия учитывающие двойные нули. Эти коэффициенты изменяются при появлении двойных нулей. Сходство возрастает за счет того, что отсутсвие признака считается тоже сходством. Пример: Коэффициенты корреляции.
...
_Важно:_ метрики неадекватно оценивают степень различия при большом количестве нулей.
...
_Коэффициент Браве-Пирсона варьирует от -1 до 1_  
_Обычно используется в R анализе_  

- Коэффициент Брея-Куртиса (Bray-Curtis dissimilarity): 
$$D = \frac{\sum \mid X_{i,j} - X_{i,k} \mid} {\sum X_{i,j} + \sum X_{i,k}}$$ 

_Это самый распространенный в экологии коэффициент._ 
...
- a - сходство объектов по наличию  
- b - различие
- c - различие
- d - сходство по отсутствию
...
$$ D = \frac{1}{p} \frac{\sum W_i\frac{\mid X_{i,j} - X_{i,k} \mid}{maxX_{i,j} - minX_{i,k}}}{\sum W_i}$$
...
## На практике для Q-анализа (в экологии и смежных дисцилинах)
...
Для бинарных данных
- Коэффициенты Сёренсена или Жаккара

## Задание: отразите графически как взаимосвязаны коэффициент Брея-Куртиса и Евклидово расстояние
...
## Задание: Постройте гистограмму, отражающую частотное распределение коэффициентов Брея-Куртиса, расчитанных для матрицы `protein`

## Решение 

```{r, fig.width = 7, fig.height = 5, fig.align = 'center'}
ggplot(distances, aes(x = Bray)) + geom_histogram(binwidth = 0.03, fill = "blue", color = "black") + theme_bw()
```

Что вы можете сказать о характере взаиморасположения объектов, глядя на это распределение?
...
>- Точки в n-мерном пространстве признаков, вероятно, формируют группы.
...
plejada <- function (data, method = "bray", Dcrit) {
  ...
  pl <- ggplot(points, aes(x = X, y = Y)) + geom_text(label = lab, size = 10) + theme_void()
  library(vegan)
  ...
  for ( i in 1:n) {
    for (j in i:n) {
      if (dist[i, j] <= Dcrit) {
        pl <- pl + geom_segment(x = points$X[i], y = points$Y[i], xend = points$X[j], yend = points$Y[j])
      }
    }
  }
  pl
...
>- Существует два подхода к анализу многоменых данных: (1) подход, основанный линейной алгебре, (2) подход, основанный на исследовании матриц сходства/различия между объектами.
