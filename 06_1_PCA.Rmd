...
>- Пусть $\mathbf{A} = cov(X) = \frac{1}{n - 1} \mathbf{X}\mathbf{X'}$ (т.е. это симметричная квадратная матрица, для нее должно существовать спектральное разложение $\mathbf{A} = \mathbf{E \Lambda E'}$).
